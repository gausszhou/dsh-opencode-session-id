 ...
 * regex that expects pure alphanumerics could miss them, so the default
 * avoids symbols; 6-bit digest values 62/63 are skipped (rejection sampling)
 * to keep the 62-char alphabet uniform.
 */
...
const NANOID_ALPHABET_URLSAFE = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_-";
/** Config value → alphabet table. */
const NANOID_ALPHABETS = { alphanumeric: NANOID_ALPHABET, urlsafe: NANOID_ALPHABET_URLSAFE };
/**
 ...
 * input always maps to the same token — across requests and process
 * restarts — so a gateway that keys on the token can attribute a dsh
 * conversation. Pure function; no randomness involved. The wire path hashes
 * the uuid only: `wireSessionId` calls `stripSessionPrefix` first, so the
 * `session-` label does not participate.
 * @param input - the string to hash.
 * @param length - token length (default 8).
 * @param alphabet - allowed characters (62–64 unique chars).
 */
function nanoidOf(input, length = 8, alphabet = NANOID_ALPHABET) {
	...
	while (out.length < length) {
		if (bit + 6 > digest.length * 8) {
			// Ran past this digest block (only reachable through heavy
			// rejection sampling): hash again with a counter for fresh bytes.
		...
		value &= 0x3f;
		bit += 6;
		if (value >= size) continue; // rejection sampling: uniform over 62 chars
		out += alphabet[value];
	}
	return out;
}
...
/**
 * Strip the dsh `session-` prefix so the wire token hashes only the uuid
 * part (`e820d21d-…-a309f722a3bc`), independent of the label. Ids without
 * the prefix (env fallback / random uuid) pass through unchanged.
 */
...
function apply(ctx, config = {}) {
	...
	const wireSessionId = (raw) => cfg.nanoidSessionId ? nanoidOf(stripSessionPrefix(raw), cfg.nanoidLength, NANOID_ALPHABETS[cfg.nanoidAlphabet]) : raw;
	const envFallback = () => {
	...
	const log = (message) => {
		...
		// The web app routes cordis logs to an invisible sink; print to stderr
		// as well so systemd/journal shows each injection live. Never stdout:
		// the acp / headless / sdk profiles carry JSON-RPC or the final answer
		// there, and a bare line breaks those channels.
		console.error(`[opencode-session-id] ${message}`);
	};
	...
	// Same reason as `log` above: stderr keeps stdout free for the profile's
	// own protocol (a client reading acp stdout rejects a non-JSON line, and
	// `--profile headless` prints its answer there).
	console.error(
		`[opencode-session-id] mounted: providers=[${cfg.providers.join(", ")}] headers=[${cfg.headers.join(", ")}] hosts=[${cfg.hosts.join(", ")}]${cfg.baseURLs.length > 0 ? ` baseURLs=[${cfg.baseURLs.join(", ")}]` : ""}${Object.keys(cfg.extraHeaders).length > 0 ? ` extra=[${Object.entries(cfg.extraHeaders).map(([h, v]) => `${h}: ${v}`).join(", ")}]` : ""}${cfg.userAgent ? ` userAgent=${cfg.userAgent}` : ""}${cfg.seedSessionId ? " seedSessionId=on" : ""}${cfg.nanoidSessionId ? ` sessionId=nanoid${cfg.nanoidLength}(${cfg.nanoidAlphabet})` : ""}`
	);
}
//#endregion

export { DEFAULT_HEADERS, NANOID_ALPHABET, NANOID_ALPHABET_URLSAFE, SessionId, apply, createSessionScope, inject, installSessionHeaderFetch, matchesTarget, name, nanoidOf, normalize, scopedIterable, stripSessionPrefix, withSessionHeaders };
