...
function apply(ctx, config = {}) {
	...
	console.log(
		`[opencode-session-id] mounted: providers=[${cfg.providers.join(", ")}] headers=[${cfg.headers.join(", ")}] hosts=[${cfg.hosts.join(", ")}]${cfg.baseURLs.length > 0 ? ` baseURLs=[${cfg.baseURLs.join(", ")}]` : ""}${cfg.seedSessionId ? " seedSessionId=on" : ""}`
	);
}
//#endregion
